		...
		inline hopp::vector_pair<std::string, hopp::vector_pair<std::string, std::string>> ini(std::string const & filename)
		{
			...
			while (file.good())
			{
				...
				if (char(file.peek()) == '[')
				{
					...
					hopp::remove_trailing_whitespaces(section);std::cout << "section = " << section << std::endl;
					hopp::ignore_line(file);
					continue;
				}
				...
				hopp::remove_trailing_whitespaces(key);std::cout << "key = " << key << std::endl;
				
				// Value
				std::string value;
				hopp::ignore_whitespaces(file);
				hopp::get_until(file, value, { ';', '#', '\n' });
				hopp::remove_trailing_whitespaces(value);std::cout << "value = " << value << std::endl;
				hopp::ignore_line(file);
